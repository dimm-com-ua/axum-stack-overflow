...
use crate::models::{Answer, AnswerDetail, AnswerId, DBError, Question, QuestionDetail, QuestionId};
...
impl HandleError {
    pub fn default_internal_error() -> Self {
        HandleError::InternalError("Internal server error! Please try again.".to_string())
    }
}
...
) -> Result<QuestionDetail, HandleError> {
    let question = questions_dao.create_question(question).await;
    match question {
        Ok(question) => { Ok(question) }
        Err(err) => {
            error!("{:?}", err);
            Err(HandleError::default_internal_error())
        }
    }
}
...
) -> Result<Vec<QuestionDetail>, HandleError> {
...
pub async fn delete_question(
    ...
    if result.is_err() {
        return Err(HandleError::default_internal_error())
    }
...
pub async fn create_answer(
...
) -> Result<AnswerDetail, HandleError> {
    ...
    match answer {
        ...
        Err(err) => {
            match err {
                DBError::InvalidUUID(s) => { Err(HandleError::BadRequest(s)) }
                DBError::Other(_) => { Err(HandleError::default_internal_error()) }
            }
        }
    }
}
...
pub async fn read_answers(
    ...
    answers_dao: &(dyn AnswersDao + Send + Sync)
) -> Result<Vec<AnswerDetail>, HandleError> {
    let answers = answers_dao.get_answers(question_id.question_uuid)
        .await;

    match answers {
        Ok(answers) => { Ok(answers) }
        Err(_) => { Err(HandleError::default_internal_error()) }
    }
}
...
pub async fn delete_answer(
    ...
    let result = answers_dao.delete_answer(answer_id.answer_uuid)
        .await;
...
mod tests {
    ...
    async fn create_question_should_return_error() {
        ...
        assert!(
            std::mem::discriminant(&result.unwrap_err())
                == std::mem::discriminant(&HandleError::InternalError("".to_owned()))
        );
    }
    ...
    async fn create_answer_should_return_bad_request_error() {
        ...
        assert!(
            std::mem::discriminant(&result.unwrap_err())
                == std::mem::discriminant(&HandleError::BadRequest("".to_owned()))
        );
    }
